...
6. **Version Bumping / Incremento Versione**: Quando lo ritieni opportuno, aumenta la versione dell'applicazione in modo unificato. Mantieni allineata e coerente la versione dell'intera applicazione (CLI, TUI, app, package e `Script.version` in `packages/script/src/index.ts`) su un'unica versione (es. `0.1.0 -> 0.1.1` per patch, bugfix o `0.2.0` per minor, nuova feature). Aggiorna `version` nei `package.json` dei package toccati o dell'app (`packages/opencode`, `packages/tui`, `packages/script/src/index.ts`, ecc.) in modo che non divergano e che `eaglecode --version` rifletta sempre la versione unica corretta. Non incrementare il major senza esplicita richiesta.
