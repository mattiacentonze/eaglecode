...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      const isBusy = yield* state.isBusy(input.sessionID)
      if (input.force === false && isBusy) {
        let q = promptQueue.get(input.sessionID)
        if (!q) {
          q = []
        ...
        const info: SessionV1.User = {
          ...
          model: {
            providerID: input.model?.providerID ?? ProviderV2.ID.make("iit"),
            modelID: input.model?.modelID ?? ModelV2.ID.make("deepseek-v4-flash"),
            variant: input.variant,
          },
          priority: false,
        }
