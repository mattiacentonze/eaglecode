...
export function createPromptSubmit(input: PromptSubmitInput) {
  ...
  const handleSubmit = async (event: Event, options?: { immediate?: boolean }) => {
    ...
    const immediate = options?.immediate || (event instanceof KeyboardEvent && (event.ctrlKey || event.metaKey))
    if (!immediate && !isNewSession && mode === "normal" && input.shouldQueue?.()) {
      input.onQueue?.(draft)
